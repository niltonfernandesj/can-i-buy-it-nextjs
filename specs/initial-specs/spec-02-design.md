> **Arquivo histórico — não editar.** Snapshot congelado do design ao final da sprint 1 (M1–M7), antes do início da sprint 2. A versão viva e atualizada fica em `specs/spec-02-design.md`.
...
Ir para mês anterior/próximo; clicar no período exibido abre um seletor dedicado (ano atual, 12 meses em grade, mês selecionado destacado, navegação entre anos, atualiza a Visão geral ao selecionar). Desktop: `Popover` do shadcn/ui a partir do período. Mobile: `Sheet` com `side="bottom"` (bottom sheet) — ambos já disponíveis na stack, sem nova dependência. Sem swipe entre meses no escopo atual.

#### 8.3.2 Resumo financeiro do mês
Três indicadores: **Entradas** (soma de todas as entradas do mês); **Saídas** (soma das Saídas no débito e das Saídas no crédito do mês de referência); **Disponível** (Entradas − Saídas no débito − Saídas no crédito). Cards apenas informativos no MVP (não são atalhos/links/expansores). No mobile: grid de duas linhas — 1ª linha Entradas e Saídas; 2ª linha Disponível ocupando a largura toda. Sem rolagem horizontal.
