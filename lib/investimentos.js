...
function ultimaLiquidacao(ativo) {
  ...
  // Desempate por `criadoEm`: ordenar só por `data` bastava enquanto a
  // liquidação era total e havia no máximo um evento. Como evento repetível
  // (M33), dois resgates no mesmo dia deixariam a ordem indefinida e o
  // remanescente escolhido seria arbitrário.
  return [...eventos].sort((a, b) => {
    const porData = new Date(b.data) - new Date(a.data);
    return porData !== 0 ? porData : new Date(b.criadoEm ?? 0) - new Date(a.criadoEm ?? 0);
  })[0];
}
 ...
 *
 * O caso sem evento cai na mesma fórmula, com a aquisição fazendo o papel de
 ...
 *
 * Par de `baseAtual` — **quanto** rende e **desde quando** saem da mesma
 * fonte. Se as duas divergirem, o rendimento corrige a base nova pelo
 * intervalo antigo, e isso não estoura erro nenhum: só devolve número maior
 * (Design §28.1).
 *
 * Sem coluna no schema: a data está na linha do evento. Materializá-la criaria
 * um segundo lugar para a mesma verdade, o oposto da decisão de saldos
 * derivados do M29.
 */
...
/**
 * Quanto a posição vale hoje: o corrigido quando já foi calculado, senão a
 * base. O `??` é o que mantém pré-fixado e IPCA+ no custo sem ramificação
 * espalhada — eles simplesmente não recebem `valor` (Requisitos §3.16.6).
 */
export function valorDaPosicao(ativo) {
  return ativo.valor ?? baseAtual(ativo);
}

export function saldoInvestido(ativos) {
  return apenasVivas(ativos).reduce((soma, ativo) => soma + valorDaPosicao(ativo), 0);
}
 ...
 * a sutileza da aquisição, abaixo. Os demais argumentos são somas já agregadas
...
export function saldoEmConta({ aportes = 0, resgates = 0, creditos = 0, debitos = 0, ativos = [] }) {
  // A aquisição debita PARA SEMPRE: o somatório percorre inclusive as
  // posições já liquidadas. O dinheiro saiu do caixa no dia da aquisição e
  // voltou pelos eventos de liquidação, possivelmente com valor diferente.
  // Somar só as vivas faria o caixa reaparecer sozinho no dia em que um
  // ativo vence.
  const aquisicoes = somar(ativos, "valorAquisicao");
  ...
  return (
    Number(aportes) - Number(resgates) - aquisicoes + recebidoEmLiquidacoes + Number(creditos) - Number(debitos)
  );
}
...
export function agruparPor(ativos, chave) {
  ...
  for (const ativo of apenasVivas(ativos)) {
    ...
    const base = valorDaPosicao(ativo);
    grupo.total += base;
