 ...
 *
 * **A janela da compra conta no M0 e não conta nas demais.** Não é simetria
 * quebrada por descuido: são convenções de contrato diferentes, e a defasagem
 * é o único sinal observável de qual delas o papel segue.
 *
 * Medido contra os três extratos, todos lidos em **01/09/2026**, com o
 * calendário real do CDI e agosto pela projeção da ANBIMA (−0,28%):
 *
 * ```
 *                  extrato    ignorando a janela    contando pro rata
 * BMG     (M-2)   7.412,03      +R$  1,52              +R$ 13,28
 * Fibra 1 (M0)    5.152,39      −R$ 24,84              +R$  1,92
 * Fibra 2 (M0)    5.039,81      −R$  1,40              +R$  1,05
 * ```
 *
 * Os dois extremos decidem em sentidos opostos, e nenhum dos dois cabe em
 * incerteza de data — todos foram lidos no mesmo dia. **A exceção do M39 não
 * era artefato**, como o M40 chegou a supor: ela é real, e vale para os títulos
 * com defasagem.
 *
 * A leitura plausível é contratual. No M0, que segue a convenção da NTN-B, o
 * preço pago já embute o VNA acumulado até ali e o comprador acumula do dia da
 * compra em diante. Num CDB com defasagem, o emissor aplica a correção só a
 * partir do aniversário seguinte. **É hipótese, não norma lida** — o que está
 * medido é o comportamento, em dois pontos (Requisitos §3.24.5).
 *
 * **Sem piso.** Um mês de deflação derruba o valor, que é o que acontece com o
 * título (§3.19.3).
 *
 * **Devolve a LISTA, não o produtório** (M40, Design §31.6). O fator é um
 * `reduce` sobre ela, logo abaixo; a lista serve também ao sinal de índice
 * ausente e — não menos importante — a depurar. Toda a investigação do M39 e
 * do M40 foi feita reconstruindo estas janelas à mão, fora do app, porque elas
 * não existiam como valor.
 */
export function janelasDeInflacao({
  indices = [],
  dataAquisicao,
  corte,
  defasagemMeses = 2,
  ...
  if (!corte || corte < dataAquisicao) return [];
  ...
  const dia = dataAquisicao.slice(8, 10);
  const mesDaCompra = dataAquisicao.slice(0, 7);

  // Onde o acúmulo começa depende da convenção do papel, e `defasagemMeses` é
  // o proxy dela. No M0, a janela que CONTÉM a compra — antes do dia 15, a que
  // abriu no mês anterior. Nos demais, a primeira que abre DEPOIS da compra;
  // comprar exatamente no dia 15 pega a janela que abre naquele dia, que não é
  // parcial.
  ...
  while (dia15(mesJanela) < corte) {
    ...
    // Só morde na primeira janela do M0, a única que pode abrir antes da
    // compra — nas demais convenções `inicio` nunca é anterior à aquisição.
    const de = inicio < dataAquisicao ? dataAquisicao : inicio;
    const vividos = diasUteis(de, fim < corte ? fim : corte);

    // Janela sem tempo vivido não é janela: não entra na lista, para não ser
    // confundida com uma que faltou índice.
    if (total > 0 && vividos > 0) {
      const mesDoIndice = deslocarMes(mesJanela, -Math.max(defasagemMeses - 1, 0));
      janelas.push({
        mes: mesJanela,
        mesDoIndice,
        fracao: vividos / total,
        valor: porMes.get(mesDoIndice),
      });
    }
...
/** O produtório das janelas que têm índice. As sem índice não rendem. */
export function fatorInflacao(indices, opcoes = {}) {
  return janelasDeInflacao({ indices, ...opcoes }).reduce(
    (fator, j) => (j.valor === undefined ? fator : fator * (1 + j.valor / 100) ** j.fracao),
    1,
  );
}

/**
 * A posição atravessou alguma janela **sem índice nenhum**?
 *
 * É o sinal que a tela usa para o ícone de atenção (Requisitos §3.24.7). Marca
 * **ausência, não provisoriedade**: uma janela servida pela projeção da ANBIMA
 * tem valor e não conta aqui — a projeção é o dado certo para o mês aberto,
 * não um substituto pior.
 *
 * Recebe os mesmos argumentos de `valorCorrigido` de propósito: quem já sabe
 * calcular o valor de uma posição não precisa aprender nada novo para saber se
 * ele está completo.
 */
export function mesesSemIndice(
  { indexador, dataAquisicao, vencimento, defasagemMeses = 2 },
  taxas = [],
  indices = [],
) {
  if (SERIE_MENSAL_DO_INDEXADOR[indexador] === undefined) return [];

  const incidentes = taxasAplicaveis(taxas, { dataAquisicao, vencimento });
  const janelas = janelasDeInflacao({
    indices,
    dataAquisicao,
    corte: incidentes.at(-1)?.dia,
    defasagemMeses,
    calendario: taxas,
  });

  // O mês DO ÍNDICE, não o da janela: é o que faltou, e é o que o aviso na
  // tela precisa nomear para o usuário entender o porquê.
  return [...new Set(janelas.filter((j) => j.valor === undefined).map((j) => j.mesDoIndice))];
}

/** O predicado, para quem só precisa saber se falta algo. */
export function inflacaoIncompleta(posicao, taxas = [], indices = []) {
  return mesesSemIndice(posicao, taxas, indices).length > 0;
}
