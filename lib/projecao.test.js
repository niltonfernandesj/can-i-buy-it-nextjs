...
describe("comporMes", () => {
  ...
  const VALORES_PADRAO = [
    { tipo: "ENTRADA", valor: 8000, meio: null },
    { tipo: "SAIDA", valor: 1000, meio: "DEBITO" },
  ...
  it("caso 2 — gasto avulso menor que o teto: estimativa = teto − avulso", () => {
    ...
    expect(resultado.debito).toEqual({ real: 300, estimado: 700, total: 1000 });
  });

  it("caso 3 — gasto avulso maior que o teto: estimativa zero, total = real", () => {
    const hoje = new Date(2026, 6, 1);
    const resultado = comporMes({
      mesReferencia: 8,
      anoReferencia: 2026,
    ...
    expect(resultado.debito).toEqual({ real: 1500, estimado: 0, total: 1500 });
  });
  ...
  it("caso 5 — ocorrência de recorrência consome o teto, como um avulso", () => {
    const hoje = new Date(2026, 6, 1);
    const resultado = comporMes({
      mesReferencia: 8,
      anoReferencia: 2026,
