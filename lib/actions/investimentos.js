...
import { baseAtual, dataBase, saldoEmConta } from "@/lib/investimentos";
import { INDEXADORES_POR_ESTRATEGIA, MOTIVOS_POR_NATUREZA } from "@/lib/ativos";
import { buscarSerie, lacunas } from "@/lib/bc";
import { buscarProjecaoIPCA } from "@/lib/anbima";
...
async function validar(dados) {
  const { contaId, estrategia, produto, emissor, indexador, taxa, dataAquisicao, vencimento, valorAquisicao, defasagemMeses } = dados;
  ...
  // Só faz sentido em indexador de inflação; nos demais o campo existe e é
  // ignorado (Design §30.1).
  const defasagem = estrategia === "INFLACAO" ? Number(defasagemMeses ?? 2) : 2;
  if (!Number.isInteger(defasagem) || defasagem < 0) {
    return { erro: "Defasagem do índice inválida." };
  }
 ...
 */
export async function liquidarAtivo(id, { data, valor, remanescente = 0 }) {
  const session = await getServerSession(authOptions);
  if (!session?.user?.id) return { error: "Não autenticado." };
  ...
  // Contra a data do último evento, não a da compra: um segundo resgate não
  // pode ser anterior ao primeiro (M33).
  const desdeQuando = dataBase(ativo);
  if (quando < desdeQuando) {
    return {
      error:
        ativo.liquidacoes.length > 0
          ? "O resgate não pode ser anterior ao resgate anterior."
          : "O resgate não pode ser anterior à aquisição.",
    };
  }
  ...
  // Zero encerra a posição — é o caso que existia sozinho até o M33.
  const sobra = Number(remanescente);
  if (!Number.isFinite(sobra) || sobra < 0) {
    return { error: "Saldo remanescente inválido." };
  }
  const base = baseAtual(ativo);
  if (sobra >= base) {
    return {
      error: `O saldo remanescente precisa ser menor que ${base.toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
      })} — acima disso não houve resgate.`,
    };
  }

  await db.liquidacaoAtivo.create({
    data: { ativoId: id, data: quando, valorRecebido: recebido, valorRemanescente: sobra },
  });
...
/**
 * Traz da API do BC só os dias que faltam e devolve a série completa da
 * janela (Design §23.2).
 *
 * **Nunca lança.** Se o Banco Central não responder, devolve o que a tabela já
 * tem — o cálculo roda com dados mais antigos e a tela não quebra (Requisitos
 * §3.16.5). É por isso que não existe "último resultado salvo": o resultado é
 * derivado da série, e a série é o que se guarda.
 */
export async function sincronizarSerie(serie, desejadoDe, desejadoAte) {
  const [maisAntiga, maisNova] = await Promise.all([
    db.taxaDiaria.findFirst({ where: { serie }, orderBy: { data: "asc" } }),
    db.taxaDiaria.findFirst({ where: { serie }, orderBy: { data: "desc" } }),
  ]);

  const comoISO = (linha) => (linha ? linha.data.toISOString().slice(0, 10) : null);

  for (const faixa of lacunas({
    desejadoDe,
    desejadoAte,
    guardadoDe: comoISO(maisAntiga),
    guardadoAte: comoISO(maisNova),
  })) {
    const { pontos, erro } = await buscarSerie(serie, faixa.de, faixa.ate);
    if (erro) {
      console.error(`[BC] ${serie} ${faixa.de}..${faixa.ate}: ${erro}`);
      continue; // segue com o que houver — a janela apenas fica incompleta
    }
    if (!pontos.length) continue;

    // createMany + skipDuplicates em vez de upsert em laço: uma ida ao banco
    // por faixa, e a PK composta é quem garante a idempotência.
    await db.taxaDiaria.createMany({
      data: pontos.map((p) => ({ serie, data: new Date(`${p.dia}T00:00:00Z`), valor: p.valor })),
      skipDuplicates: true,
    });
  }

  const linhas = await db.taxaDiaria.findMany({
    where: { serie, data: { gte: new Date(`${desejadoDe}T00:00:00Z`), lte: new Date(`${desejadoAte}T00:00:00Z`) } },
    orderBy: { data: "asc" },
  });

  // Devolve no formato que lib/rendimento.js espera: dia em string ISO, para
  // a comparação com dataAquisicao não passar por fuso nenhum.
  return linhas.map((l) => ({ dia: l.data.toISOString().slice(0, 10), valor: Number(l.valor) }));
}

/**
 * Mesma mecânica de `sincronizarSerie`, mas para um índice **mensal**
 * (Design §29.2).
 *
 * `buscarSerie` e `lacunas` são reaproveitados sem alteração: o payload tem a
 * mesma forma e as lacunas operam sobre strings ISO, sem saber de
 * granularidade. O que muda é a tabela de destino e o nome do campo — `mes`,
 * não `data`, porque o nome carrega a granularidade.
 */
export async function sincronizarIndice(serie, desejadoDe, desejadoAte) {
  // A janela chega com precisão de DIA, vinda da aquisição mais antiga da
  // carteira. Um índice mensal é datado no dia 1º, então filtrar por
  // "2026-01-12" descartaria janeiro inteiro — justamente o mês da compra, que
  // conta (Requisitos §3.19.2). Alinhar ao primeiro dia do mês corrige tanto o
  // que se busca quanto o que se devolve.
  const inicio = `${desejadoDe.slice(0, 7)}-01`;

  const [maisAntigo, maisNovo] = await Promise.all([
    db.indiceMensal.findFirst({ where: { serie }, orderBy: { mes: "asc" } }),
    db.indiceMensal.findFirst({ where: { serie }, orderBy: { mes: "desc" } }),
  ]);

  const comoISO = (linha) => (linha ? linha.mes.toISOString().slice(0, 10) : null);

  for (const faixa of lacunas({
    desejadoDe: inicio,
    desejadoAte,
    guardadoDe: comoISO(maisAntigo),
    guardadoAte: comoISO(maisNovo),
  })) {
    const { pontos, erro } = await buscarSerie(serie, faixa.de, faixa.ate);
    if (erro) {
      console.error(`[BC] ${serie} ${faixa.de}..${faixa.ate}: ${erro}`);
    ...
    await db.indiceMensal.createMany({
      data: pontos.map((p) => ({ serie, mes: new Date(`${p.dia}T00:00:00Z`), valor: p.valor })),
      skipDuplicates: true,
    });
  }

  const linhas = await db.indiceMensal.findMany({
    where: { serie, mes: { gte: new Date(`${inicio}T00:00:00Z`), lte: new Date(`${desejadoAte}T00:00:00Z`) } },
    orderBy: { mes: "asc" },
  });

  return linhas.map((l) => ({ mes: l.mes.toISOString().slice(0, 10), valor: Number(l.valor) }));
}

/**
 * Janela em que uma projeção já lida continua servindo sem nova consulta.
 *
 * A ANBIMA revisa o número algumas vezes por mês; a tela é aberta muito mais
 * que isso. Doze horas é consideração com um site público que não pediu para
 * ser cliente deste app, e é o que impede uma indisponibilidade de virar
 * espera de 15s em todo carregamento (Design §31.4).
 */
const VALIDADE_PROJECAO_MS = 12 * 60 * 60 * 1000;

/**
 * Espera depois de uma falha, antes de tentar de novo.
 *
 * Sem isto, ANBIMA fora do ar significaria uma tentativa — e o timeout inteiro
 * — a **cada render**, porque a falha não atualiza `capturadoEm` e o cache de
 * 12h nunca fecha. O estado é de processo: reinício zera, e cada instância tem
 * o seu. É proteção contra o caso comum, não garantia distribuída.
 */
const ESPERA_APOS_FALHA_MS = 30 * 60 * 1000;
const proximaTentativa = new Map();

/** Linha do banco → o mesmo formato que `sincronizarIndice` devolve. */
function comoPonto(linha) {
  return linha && { mes: linha.mes.toISOString().slice(0, 10), valor: Number(linha.valor) };
}

/**
 * A projeção corrente do índice, lida da ANBIMA e guardada (Design §31.4).
 *
 * **Nunca interrompe o carregamento da tela.** Falha de rede, página mudada ou
 * timeout devolvem o que já estiver guardado — inclusive `null`. Quem chama
 * trata a ausência como "mês não publicado", que é situação que o cálculo já
 * sabe atravessar desde o M36 (Requisitos §3.24.6).
 *
 * Devolve `{ mes, valor }` ou `null`.
 */
export async function sincronizarProjecao(serie) {
  // Só o IPCA tem projeção publicada nessa página. Um índice novo entraria
  // aqui como caso próprio, não por acaso.
  if (serie !== "IPCA") return null;

  const guardada = await db.projecaoIndice.findFirst({
    where: { serie },
    orderBy: { mes: "desc" },
  });

  const agora = Date.now();
  if (guardada && agora - guardada.capturadoEm.getTime() < VALIDADE_PROJECAO_MS) {
    return comoPonto(guardada);
  }
  if (agora < (proximaTentativa.get(serie) ?? 0)) return comoPonto(guardada);

  const { projecao, erro } = await buscarProjecaoIPCA();
  if (erro) {
    console.error(`[ANBIMA] ${serie}: ${erro}`);
    proximaTentativa.set(serie, agora + ESPERA_APOS_FALHA_MS);
    return comoPonto(guardada);
  }

  proximaTentativa.delete(serie);

  // Uma linha por mês, sobrescrita a cada captura: o histórico de revisões da
  // ANBIMA não interessa, só o valor vigente (Requisitos §3.24.8).
  const mes = new Date(`${projecao.mes}T00:00:00Z`);
  const conteudo = { valor: projecao.valor, fechado: projecao.fechado, capturadoEm: new Date(agora) };
  await db.projecaoIndice.upsert({
    where: { serie_mes: { serie, mes } },
    update: conteudo,
    create: { serie, mes, ...conteudo },
  });

  return { mes: projecao.mes, valor: projecao.valor };
}
