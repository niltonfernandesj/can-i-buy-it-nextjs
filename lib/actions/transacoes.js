...
import { gerarOcorrenciasRecorrencia } from "@/lib/recorrencia";
...
// Uma parcela ou ocorrência recorrente tem dataEfetiva/mesReferencia definidos
// pelo gerarParcelas/gerarOcorrenciasRecorrencia (Tasks 8 e 29), não pela própria
// dataCompra — reaplicar o cálculo de fatura genérico aqui corromperia essa
// sequência. Identifica a qual dos dois grupos (se algum) uma transação pertence.
function grupoDaTransacao(transacao) {
  ...
  if (transacao.recorrenciaId !== null) {
    return { campo: "recorrenciaId", id: transacao.recorrenciaId };
  }
  return null;
}
...
export async function editarTransacao(id, dados, { propagarParaRestantes } = {}) {
  ...
  // Numa parcela ou ocorrência recorrente, só o conteúdo (valor/descrição/categoria)
  // é editável; conta, data e tipo ficam travados no que já existe.
  ...
  if (propagar) {
    ...
    // dataEfetiva futura do mesmo grupo (parcelamento ou recorrência).
...
export async function apagarTransacao(id, { propagarParaRestantes } = {}) {
  ...
  if (propagar) {
    // A própria linha + todas as de dataEfetiva futura do mesmo grupo
...
async function validarTransacaoRecorrente({
  tipo,
  descricao,
  categoria,
  contaId,
  ...
  const n = Number(numeroMeses);
  if (!Number.isInteger(n) || n < 2) {
    return { erro: "Número de meses deve ser um inteiro maior ou igual a 2." };
  }
  ...
  if (tipo === "SAIDA") {
    if (conta.tipo !== "CONTA_CORRENTE" && conta.tipo !== "CARTAO_CREDITO") {
      return { erro: "Saída recorrente só é permitida em conta corrente ou cartão de crédito." };
    }
  } else {
    if (conta.tipo !== "CONTA_CORRENTE") {
      return { erro: "Entrada recorrente só é permitida em conta corrente." };
    }

    if (investimento) {
      return { erro: "Entrada recorrente não pode ser marcada como investimento." };
    }
  }
  ...
  if (investimento) {
    if (conta.tipo !== "CONTA_CORRENTE") {
      return {
        erro:
          "Aporte de investimento só pode ser lançado numa conta corrente (a conta de destino é indicada separadamente).",
      };
    }

    if (!contaInvestimentoId) {
      return { erro: "Selecione a conta de investimento de destino." };
    }
...
export async function criarTransacaoRecorrente(dados) {
  const session = await getServerSession(authOptions);
  if (!session?.user?.id) {
    return { error: "Não autenticado." };
  }

  const { erro, valores } = await validarTransacaoRecorrente(dados);
  if (erro) {
    return { error: erro };
  }
  ...
  const ocorrencias = gerarOcorrenciasRecorrencia(dataCompra, valor, numeroMeses, conta);
