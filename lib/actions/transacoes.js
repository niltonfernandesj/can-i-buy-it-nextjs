...
import { ehFutura, paraDataLocal as paraData } from "@/lib/datas";
 ...
 *
 * `obrigatoria` é falso para aporte e resgate, que não têm categoria (Design
 * §21.2). A coluna virou nullable no M34 — a trava que resta é esta aqui.
 */
async function resolverCategoria(categoriaId, idAtual, obrigatoria = true) {
  if (!categoriaId) {
    return obrigatoria
...
async function validarTransacao({
  ...
  // Só investimento. Transação comum aceita data futura de propósito — é o
  // que sustenta despesa agendada e a Projeção. A trava existe aqui porque
  // editar um aporte em /transacoes é a única porta que sobrou para empurrar
  // a data de um lançamento de investimento para o futuro (Requisitos
  // §3.13.5). validarTransacaoParcelada tem um bloco idêntico logo abaixo e
  // NÃO recebe a trava: parcelamento é exclusivo de saída no cartão.
  if (investimento && ehFutura(dataCompraObj)) {
    return { erro: "Aporte e resgate não podem ter data futura — investimento se registra depois que acontece." };
  }
