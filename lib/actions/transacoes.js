...
export async function editarTransacao(id, dados, { propagarParaRestantes } = {}) {
  ...
  // Uma parcela tem dataEfetiva/mesReferencia definidos pelo gerarParcelas (Task 8),
  // não pela própria dataCompra — reaplicar o cálculo de fatura genérico aqui
  // corromperia essa sequência. Então só o conteúdo (valor/descrição/categoria) é
  // editável numa parcela; conta, data e tipo ficam travados no que já existe.
  const ehParcela = existente.parcelamentoId !== null;

  const { erro, valores } = ehParcela
  ...
  const propagar = Boolean(propagarParaRestantes) && ehParcela;

  if (propagar) {
    // A própria parcela (dataEfetiva = existente.dataEfetiva) + todas as de
    // dataEfetiva futura do mesmo parcelamento.
    await db.transacao.updateMany({
      where: {
        parcelamentoId: existente.parcelamentoId,
...
export async function apagarTransacao(id, { propagarParaRestantes } = {}) {
  ...
  const propagar = Boolean(propagarParaRestantes) && existente.parcelamentoId !== null;

  if (propagar) {
    // A própria parcela + todas as de dataEfetiva futura do mesmo parcelamento.
