...
export async function apagarValorPadrao(id) {
  ...
  // Consolidações (spec-01 §3.8) referenciam o item via FK RESTRICT — sem
  // apagá-las primeiro, apagar um item consolidado falha com violação de
  // integridade referencial. Transação garante que as duas exclusões
  // aconteçam juntas ou nenhuma aconteça.
  await db.$transaction([
    db.consolidacaoValorPadrao.deleteMany({ where: { valorPadraoId: id } }),
...
export async function consolidarValorPadrao({ valorPadraoId, mesReferencia, anoReferencia, valor }) {
  const session = await getServerSession(authOptions);
  if (!session?.user?.id) {
    return { error: "Não autenticado." };
  }
  ...
  await db.consolidacaoValorPadrao.upsert({
    where: {
...
export async function removerConsolidacaoValorPadrao({ valorPadraoId, mesReferencia, anoReferencia }) {
  const session = await getServerSession(authOptions);
  if (!session?.user?.id) {
    return { error: "Não autenticado." };
  }

  await db.consolidacaoValorPadrao.deleteMany({
