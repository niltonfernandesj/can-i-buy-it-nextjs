...
import { enumLegadoDaCategoria } from "@/lib/categorias";

const TIPOS_VALIDOS = ["ENTRADA", "SAIDA"];
const MEIOS_VALIDOS = ["CREDITO", "DEBITO"];

// Mesma escrita dupla de lib/actions/transacoes.js (Design §18.2): grava
// categoriaId e a coluna enum legada enquanto ela existir. `idAtual` permite
// que uma categoria já inativa continue valendo numa edição (Design §18.3).
async function resolverCategoria(categoriaId, idAtual) {
  ...
  if (!categoria.ativa && categoriaId !== idAtual) {
    return { erro: `A categoria "${categoria.nome}" está inativa e não pode ser usada.` };
  }

  return {
    campos: { categoriaId: categoria.id, categoria: enumLegadoDaCategoria(categoria.nome) },
  };
}
...
async function validarValorPadrao({ descricao, valor, tipo, meio, categoriaId }, categoriaIdAtual) {
  if (!descricao?.trim()) {
  ...
  if (tipo === "SAIDA") {
    ...
    const { erro: erroCategoria, campos } = await resolverCategoria(categoriaId, categoriaIdAtual);
    if (erroCategoria) {
      return { erro: erroCategoria };
    }
    return { valores: { descricao: descricao.trim(), valor: valorNum, tipo, meio, ...campos } };
  }

  // Receita padrão não tem categoria — as duas colunas ficam nulas.
  return {
    valores: {
...
export async function criarValorPadrao(dados) {
  ...
  const { erro, valores } = await validarValorPadrao(dados);
  if (erro) {
    return { error: erro };
  }
...
export async function editarValorPadrao(id, dados) {
  ...
  const { erro, valores } = await validarValorPadrao(dados, existente.categoriaId);
  if (erro) {
    return { error: erro };
  }
...
export async function consolidarDespesaPadrao({
  ...
  // A categoria vem pré-preenchida do item de despesa padrão, que pode ter
  // sido desativada depois — por isso o id do próprio item é o "atual".
  const { erro: erroCategoria, campos: camposCategoria } = await resolverCategoria(
    categoriaId,
    item.categoriaId
  );
  if (erroCategoria) {
    return { error: erroCategoria };
  }
