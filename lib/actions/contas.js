...
export async function editarConta(id, { nome, tipo, diaFechamento, diaVencimento }) {
  ...
  const contaExistente = await db.conta.findUnique({ where: { id } });
  if (!contaExistente) {
  ...
  // tipo/diaFechamento/diaVencimento já determinaram o mesReferencia das
  // transações existentes (Design §17.4) — alterá-los depois corromperia a
  // consolidação silenciosamente. Ficam imutáveis assim que a conta tem
  // qualquer transação vinculada; só o nome continua editável.
  const totalTransacoes = await db.transacao.count({ where: { contaId: id } });

  if (totalTransacoes > 0) {
    const configuracaoMudou =
      valores.tipo !== contaExistente.tipo ||
      valores.diaFechamento !== contaExistente.diaFechamento ||
      valores.diaVencimento !== contaExistente.diaVencimento;

    if (configuracaoMudou) {
      return {
        error:
          "Esta conta já tem transações lançadas: tipo, dia de fechamento e dia de vencimento não podem mais ser alterados — eles já determinaram o mês de referência dos lançamentos existentes. Apenas o nome pode ser editado.",
      };
    }
  }
